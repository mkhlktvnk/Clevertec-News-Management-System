...
SELECT 'user1', 'I totally agree with this study!', 1
WHERE EXISTS(SELECT 1 FROM news WHERE id = 1);

INSERT INTO comments (username, text, news_id)
SELECT 'user2', 'I am hopeful for the future of renewable energy!', 2
WHERE EXISTS(SELECT 1 FROM news WHERE id = 2);

INSERT INTO comments (username, text, news_id)
SELECT 'user3', 'Digital connectivity has been a lifesaver during the pandemic.', 3
WHERE EXISTS(SELECT 1 FROM news WHERE id = 3);

INSERT INTO comments (username, text, news_id)
SELECT 'user4', 'AI technology is fascinating, but I hope it doesnt replace human doctors.', 4
WHERE EXISTS(SELECT 1 FROM news WHERE id = 4);

INSERT INTO comments (username, text, news_id)
SELECT 'user5', 'Its amazing how much we can learn from fossils.', 5
WHERE EXISTS(SELECT 1 FROM news WHERE id = 5);

INSERT INTO comments (username, text, news_id)
SELECT 'user6', 'Ive always been skeptical of artificial sweeteners.', 6
WHERE EXISTS(SELECT 1 FROM news WHERE id = 6);

INSERT INTO comments (username, text, news_id)
SELECT 'user7', 'This study provides important information about the risks of artificial sweeteners.', 6
WHERE EXISTS(SELECT 1 FROM news WHERE id = 6);

INSERT INTO comments (username, text, news_id)
SELECT 'user8', 'I think we need to prioritize mental health as much as physical health.', 7
WHERE EXISTS(SELECT 1 FROM news WHERE id = 7);

INSERT INTO comments (username, text, news_id)
SELECT 'user9', 'This pandemic has shown us just how important it is to have a strong healthcare system.', 8
WHERE EXISTS(SELECT 1 FROM news WHERE id = 8);

INSERT INTO comments (username, text, news_id)
SELECT 'user10', 'I hope this new technology can improve access to healthcare for everyone.', 4
WHERE EXISTS(SELECT 1 FROM news WHERE id = 4);

INSERT INTO comments (username, text, news_id)
SELECT 'user11', 'I am excited to see what new discoveries paleontologists will make in the future.', 5
WHERE EXISTS(SELECT 1 FROM news WHERE id = 5);

INSERT INTO comments (username, text, news_id)
SELECT 'user12', 'We should all do our part to take care of the environment.', 2
WHERE EXISTS(SELECT 1 FROM news WHERE id = 2);
